...
- **Track B — FalkorDB graph backend (D044, D056):** **PR1 SHIPPED → PR #193 OPEN** (`stores/falkor-graph-store`, `52db580`; off `origin/main`). The offline **parity FLOOR**: a FalkorDB (Redis/openCypher) sibling of `Neo4jGraphStore`, parity-by-construction (search delegates to a transient in-memory `GraphStore`), MATCH-never-MERGE (no D041 bug), lazy/fail-loud, stdlib floor preserved. 28 parity tests + full stores suite 434/0-fail; cross-vendor gated (architect claude → implementer codex → verifier claude PASS 6/6). Built in the `amh-falkor` worktree, NOT the live pipeline checkout. **PR2 (native=True typed-graph + captained measurement via `falkordblite`) DEFERRED** — runs after the accum run finishes + deps installed. See `GRAPH_STORE_SCOPE.md` + D056.
