...
- **✅ CHOSEN SEQUENCE: `pydata_xarray_sequence` (22 tasks, py3.10).** Gold patch grades **RESOLVED on current `main` with ZERO grader changes** — `needs_seed=False`, plain `pip install -e .`, plain pytest, no tox, no compiled deps. `sympy` (50 tasks) is the clean backup (also RESOLVED, zero changes).
- **⭐ TOP NEXT MOVE (Brent runs `make pipeline` in his terminal):** smoke first, then full base+plugin:
  - smoke: `make pipeline ARGS="--benchmark swe_bench_cl --sequence pydata_xarray_sequence --stage base,plugin-accum --limit 3 --budget-usd 8"` (confirms the full agent→patch→grade chain; the GRADER half is already gold-patch-verified)
  - full: `make pipeline ARGS="--benchmark swe_bench_cl --sequence pydata_xarray_sequence --stage base,plugin-accum --budget-usd <N>"` — on a branch off main per [[branch-pipeline-runs]]; default budget **$10** (cost.py; too-low aborts mid-run).
