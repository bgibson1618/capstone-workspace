...
### ⭐ OVERNIGHT sphinx run — DIAGNOSED + GRADER FIX SHIPPED (2026-06-25 AM)
**The overnight `sphinx-doc_sphinx_sequence` BASE run graded 0/44 — but that was a host-env GRADING ceiling, NOT P3 code and NOT a model result.** Diagnosed deterministically (gold patch through the real grader, no LLM): `SwebenchHostGrader` builds a host `uv` venv ≠ SWE-bench's conda base, so 42/44 were UNGRADED ("official parser produced no statuses"). `$0.00` cost is normal (subscription auth, no dollar meter). Full root-cause chain + repro in memory `swebench-host-grader-env-ceiling` (scripts: capstone scratchpad `grade_diag*.py`).
- **FIX SHIPPED — PR #167 MERGED to `main` (`6a0b0b4`), repo-SCOPED.** `uv venv --seed` (pip) + era-pins (`setuptools<60`, `docutils<0.16`) for **sphinx < 4.0 only**, gated to `_CONDA_BASE_REPOS={sphinx}` so **non-sphinx grading is byte-identical → zero teammate blast radius** (Brent is the only one running sphinx; teammates run other sequences). Cross-vendor codex gate PASS on exactly that claim; 14 grader tests + 98/0/1 smoke green on merged main.
- **Now gradeable on `main`: sphinx 3.0–3.5 (22/44).** sphinx **4.x–7.x (22/44) still UNGRADED** — a SEPARATE tox-current-env wall (tox builds an isolated `.tox/py39` without pytest); flagged in the PR as an eval-infra follow-up **cc @kenhuangus** (not a dep pin).
- **⭐ TOP NEXT MOVE:** re-run sphinx with **base AND plugin-accum** stages (the overnight run was base-only → no memory delta) to get the actual with/without-memory number on the 22 gradeable tasks. Command shape: `make pipeline ARGS="--benchmark swe_bench_cl --sequence sphinx-doc_sphinx_sequence --stage base,plugin-accum --budget-usd <N>"` (default budget **$10**, cost.py; too-low aborts mid-run). Run on a branch off main per [[branch-pipeline-runs]]. *(Caveat: Haiku on sphinx is hard — base may resolve ~0 even graded; the delta is the point.)* Then decide: tackle the 4.x+ tox wall (Ken) for the full 44, or run other sequences.
