...
*All four owned components + the durable routing eval are logged. **Shipped/merged:** cascade (D008 — PR1 #17, PR2 #23); profiles+matrix (D016 — PR2.5 #27); eval growth (D018 — #28); cheap-fix router rules (#29); PR3a bake-off harness (#34); PR3b-1 Voyage embedder (#41, live-validated). **Now:** D019 — retrieval eval needs semantic cases to measure the embedder. **Next:** PR3b-2 (semantic-router classifier, Voyage encoder); optional spaCy adapter. **Considered & scoped-out:** fusion-by-default (D015), IRCoT (D017). **Future:** semantic retrieval eval / real benchmarks; router↔harness integration with Keith.*
