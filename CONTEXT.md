...
## ⭐ 2026-06-27 SESSION — read this FIRST (most recent)
**Theme: recall precision + observability, then a methodology correction.** All MERGED to `main` this session unless noted.

### ⭐ OPEN PRs + next task (explicit)
**Only ONE PR open: #241** (seed-telemetry-isolation, gate-pending). **A pre-merge consumer sweep caught the first commit was a NO-OP** (globbed the `_memory` level; telemetry lives at `.cookbook-memory/`) + a false-green test — FIXED in `a72b5a3` (glob → `.cookbook-memory/`, test rewritten to the real nested layout, pre-existing seed test updated to assert `.seed`, `.gitignore` += `*.seed`). Sweep confirmed both runtime failure modes clean (no missing-data reader, no double-count glob). **#241 still needs a final codex gate, THEN merge.** Everything else from this arc is **MERGED**: #206 (ADR), #214 (grader mirror), #216 (5ca7d7d evidence), #220 (fusion-falkor), #231 (recall UI), #232 (V5 evidence), #234 (recall floor), #236 (⚠ reconcile). **NEXT TASK = codex-gate #241 then merge** (a subagent is sweeping jsonl consumers first — confirm SAFE before merge). The embedded Falkor wiring plan (`work/falkor-wiring/wiring-plan.md`) is **RETIRED/superseded** by the external-Docker `fusion-falkor` that shipped in #220 — banner added. **Clean-rerun recipes** for the filler + accumulation experiments live in memory [[overnight-sympy-memory-experiments]] + [[fusion-falkor-measurement-recipe]] (re-run only AFTER #241 merges so telemetry is un-confounded).

### Recall thread — observability + precision fix (the team's "retrieval too eager" finding)
- **✅ #231 MERGED — recall telemetry UI**: a "Recalls" sub-tab in the Monitor (per-recall query/n/top-score/ids + score histogram + ⚠ low-confidence), **Print Report** (per-run JSON+MD export — kills the 3-subdir dig), inspector **auto-refresh** (3s), and a `run.sh` stale-bytecode guard. Surfaces recall data that was already emitted but never shown.
- **✅ #234 MERGED — configurable recall score floor** (the precision fix the team was blocked on): `RouterConfig.recall_min_score`, applied as the OUTERMOST `Router.route` wrap (post rerank/fusion/cascade). **accuracy profile auto-gets 0.15** (calibrated: n=52 real Voyage recalls — garbage top ≤0.09, real ≥0.19; floor sits in the gap). `RECALL_MIN_SCORE` env overrides any profile (`<=0` disables; typo falls through). Default unset (eval) byte-identical. Also stamps **profile + min_score into each recall event** (observability). **Teammates launch accuracy/Voyage runs and get 0.15 automatically — no setup.**
- **✅ #236 MERGED — reconcile the UI ⚠ flag** with the live floor (was a stale 0.30 guess → now reads each recall's recorded `min_score`, fallback calibrated 0.15).
- **⏭ Remaining recall levers (NOT started):** component tags · second-pass recall after code search.

### ⚠️ Overnight experiments — INCONCLUSIVE due to a confound I then FIXED
Queued two sympy experiments (distractor-robustness: +5k filler; accumulation-lift: 183 consolidated memories). **Reliable finding: sympy resolution is SATURATED at 19/22 regardless of memory** (baseline / +5k filler / consolidated all 19/22). **BUT the recall-quality conclusions were INVALID** — `--source-memory` did a full copytree that **inherited the seed store's `events.jsonl` (+ dream diaries)**, so the run's recall telemetry was the SEED's recalls, not the run's (recall events are `ts=0`, unseparable). I over-claimed "robust to filler / consolidated inert" off this confound, then caught + corrected it. See [[overnight-sympy-memory-experiments]].
- **✅ FIX — PR #241 OPEN (eval/seed-telemetry-isolation, `9afa0c5`): `_copy_memory_dataset` now archives the seed's telemetry logs to `*.seed`** after copying (memories carry forward, run starts with a clean event log). Tested; full `test_pipeline.py` green. **⛔ GATE PENDING** (not yet codex-gated — reboot was imminent). **FIRST TASK NEXT SESSION: codex-gate #241, then merge.** Then the filler + accumulation experiments are **re-runnable cleanly** (stores valid; recipes in the memory).
- **Methodology lesson for the team:** ANY prior `plugin-accum` recall analysis is likely confounded the same way — re-read post-#241.

### Infra / housekeeping
- **TODO: `make ui` monitor only watches `results/`** — custom `--results-dir` siblings (the experiments) aren't auto-discovered. Fix: accept arbitrary/multiple roots, OR write experiment results under `results/`.
- Runner clean (0 pipeline procs). Diagnostic result dirs at repo root (`results-filler-*`, `results-consolidated-allsympy*`, `results-probe-knowngood`) — local scratch, not committed; clean up.
- **Parked lanes (recoverable):** `amh-router-opt` (local cross-encoder reranker WIP, commit `e36a884`, never gated/PR'd) · `amh-scale-eval` (50k-scale eval phase-1 deliverable as a patch in its run dir). Both ~20 commits behind main → rebase on resume.
- **Worktrees:** runner + `amh-main` @ `cf83e20`; `amh-seedfix` (#241); `amh-lowconf` (#236 MERGED → prunable); `amh-router-opt` + `amh-scale-eval` (parked).

---

## 2026-06-26 SESSION (prior — superseded by the 2026-06-27 block above)
...
- **⭐ SINGLE NEXT DECISION (in flight): how to deploy the graph backend in the plugin.** Wiring FalkorDB native into `build_store` is planned (`work/falkor-wiring/wiring-plan.md` — opt-in `fusion-falkor` profile, switchable, Keith's `contract.py` seam) BUT the **embedded `redislite` model spawns a redis-server subprocess with no `close()` chain → orphaned processes** (the fix = a cross-cutting close chain). **✅ RESOLVED (researcher-qrp4, output at `work/falkor-research/`): use EXTERNAL-Docker-FalkorDB** — `FalkorGraphStore(url="redis://localhost:6379", native=True)` against `docker run falkordb/falkordb`. Connecting (not spawning) **avoids the orphan-process problem AND keeps the built+measured native**; **do NOT pivot to Neo4j** for lifecycle alone (its Phase-B native accuracy is unbuilt + live-validation-pending). This **dissolves the close-chain complexity** that prompted the question — only a redis client connection to close (no subprocess to reap; the embedded `redislite` even atexit-cleans, but external is cleaner). **NEXT = the `fusion-falkor` wiring arc using `url=` (NOT embedded)** — much lighter than the embedded `work/falkor-wiring/wiring-plan.md` (skip the cross-cutting close chain): the opt-in profile in `contract.py` (coordinate w/ Keith) + run `fusion` vs `fusion-falkor` on a pipeline slice with a Docker FalkorDB up. Per [[ask-before-assuming-unavailable]]: Docker was wrongly assumed unavailable, which forced the embedded model + this whole detour.
- **✅ GRADER LOCAL-MIRROR — PR #214 OPEN (2026-06-26, before the Falkor wiring).** Per-task SWE-bench checkouts fetched each repo fresh from github at run time → a transient github connection timeout silently UNGRADED a task (real run: `builtin` graded 21/22 vs the paired 22/22) and, agent-side, failed the whole task. Opt-in **`MEMEVAL_REPO_CACHE`** routes ALL FOUR checkout consumers (both graders + both agent-side: claudecode/cursorcli) through ONE shared `checkout_with_cache()` helper → network-free local **bare-mirror** checkout, auto-created once (flock-guarded), retry + fail-open fallback to github. Default unset = byte-identical. Cross-vendor gated (codex FAIL→fix→PASS; caught a fail-open hole + an unexpanded-`~` bug). 93 pass/2 pre-existing. **RECIPE for future runs:** set **`MEMEVAL_REPO_CACHE=~/.cache/memeval/swebench-repos`** (stable, outside every worktree); the **xarray mirror is already pre-warmed** there (140M `pydata__xarray.git`; the timed-out commit checks out in 0.1s). Other sequences (sympy…) auto-mirror on first use. Branch worktree `amh-grader-mirror` (keep until #214 merges). cc @kenhuangus (eval-infra; touches root `.env.example`). See [[swebench-repo-mirror-cache]].
- **✅ FALKOR WIRED INTO THE PLUGIN — PR #220 OPEN (2026-06-26).** `$MEMORY_PROFILE=fusion-falkor` in `contract.build_store` = fusion routing + ONLY the graph backend swapped to **external** `FalkorGraphStore(url=$FALKORDB_URL, native=True)` (connects to a Docker FalkorDB, never spawns — the researcher-qrp4 external model; fail-loud, lazy, default byte-identical). Codex-gated PASS. **External Docker FalkorDB VALIDATED** end-to-end: live parity + native structural + convergent-hub native-beats-baseline tests pass against the container (isolated client venv `~/.cache/memeval/falkor-venv`), AND `build_store` fusion-falkor + a 1-task pipeline e2e smoke both green. **⭐ MEASUREMENT IN FLIGHT (2026-06-26 eve):** `fusion` vs `fusion-falkor` (plugin-blank, --limit 5) running in the **`amh-falkor-wiring` worktree** (its own .venv + falkordb + copied/reseeded sandbox; concurrent with the v5 run, isolated) → `results-fusion/` vs `results-falkor/`. **CAVEAT:** small plugin-blank barely exercises the graph — the real graph-backend verdict needs plugin-accum (seeded graph) or scale. Recipe: [[fusion-falkor-measurement-recipe]]. NEXT after the number lands: decide accum/scale run + merge #220.
