...
**⭐ SINGLE NEXT TASK:** **re-run the full `make pipeline`** on `stores/backend-upgrades` for the first *trustworthy* accuracy number — **but only once the team's in-flight grader fixes land** (merge `main` again first). Until then, do NOT run (Brent's call). Per the Eval philosophy above, the SWE-Bench-CL pipeline is the only metric that decides anything. *(Disambiguation: this is the **local `make pipeline`** experiment-loop metric on our branch — **distinct** from the team's GitHub Actions **"Benchmark run"** workflow (the separate, downstream, team-level PRD-submission gate; see "What's next" #2). "Grader fixes landed" = **Brent's go-signal** (new grader PRs merged to `main` past #132); the specific PRs aren't pushed yet, so this is intentionally gated on his word — not a successor's hunt.)*

**Pipeline how-to:** `cd agent-memory-harness && make setup` once; the run drives the `claude` CLI against an isolated `eval/.claude-sandbox` (one-time `/login`); always pass a small `--budget-usd`; OpenRouter + Voyage keys live in `agent-memory-harness/.env` (**authoritative for runs**; a `capstone-workspace/.env` mirror also exists). **Known-open (NOT blockers for the next task):** the eval-harness `MemoryFramework.{write,get,search,all}` stubs stay bypassed by `build_store` (wire-or-retire, undecided); the capstone evidence/closeout artifact is not defined yet.
