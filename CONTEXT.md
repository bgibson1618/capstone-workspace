...
- **⭐ STATUS: the full xarray run is IN FLIGHT** (Brent's own terminal, 2026-06-25 PM). The `--limit 3` smoke already PASSED the chain end-to-end (base 1/3 resolved + 3/3 graded; plugin-blank wrote 2 memories with a recall hit). **NEXT MOVE = read the full result** under `agent-memory-harness/results/v*pydata_xarray*` (scoreboard: `cd eval && python -m memeval.results summary --path <results.json>`); the **base-vs-plugin delta is the headline number**.
...
- **Track D FTS5 lexical backend — PR #174 MERGED** (`2730041`) + **CodeRabbit follow-up PR #176 OPEN** (`stores/track-d-fts5-followup`, `4b677f4`): stdlib SQLite-FTS5 `Fts5Store`; **beats markdown lexical** (recall@10 0.842 vs 0.825, ~5× faster writes), default-eligible; production seam DEFERRED (D044). Cross-vendor gated PASS; #176 fixes CodeRabbit's 6 findings (rebuild-guard Major, reporter tmp-root/live-paths Majors, 3 minors) — 16 fts5 + 394 stores OK. (D051/D052)
