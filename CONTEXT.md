...
### ⭐ OVERNIGHT (2026-06-25, "let it cook") — the sphinx `make pipeline` run is LIVE
**Brent kicked off the SWE-Bench-CL `sphinx-doc_sphinx_sequence` (44 tasks) pipeline run in his own terminal overnight.** *(Assignment changed: Brent now runs the **sphinx** benchmark specifically.)* Stage is almost certainly `plugin-accum` (the default); the exact `--budget-usd` / `--limit` are Brent's — he ran it directly (NOT a roster run), so this session can't see the args or progress.
- **⭐ MORNING FIRST MOVE: check the run's outcome.** Results land under `agent-memory-harness/results/vbranch-<branch>-<commit>/`; scoreboard via `cd agent-memory-harness/eval && python -m memeval.results summary --path <results.json>` (or inspect the newest `results/v*/` dir + its run log). This is the **headline metric run the whole project gates on** — read the grade, then decide next (run `--stage base` over sphinx for the with/without-memory delta, run more sequences, or analyze).
- **How sphinx is selected:** it's a SEQUENCE under `--benchmark swe_bench_cl` (not a top-level benchmark — that confused the menu): `make pipeline ARGS="--benchmark swe_bench_cl --sequence sphinx-doc_sphinx_sequence --stage plugin-accum --budget-usd <N>"`. Dataset present (44 tasks verified). Default budget is **$10** (cost.py; too-low aborts mid-run). Sandbox already logged in (`eval/.claude-sandbox/.credentials.json`).
- **Console caveat:** it may have died over the overnight/date boundary — re-run `agent-roster console --ensure --workspace <ws>` if runs aren't visible (memory `reensure-console-after-boundary`).
