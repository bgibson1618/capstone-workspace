...
- **Shipped:** `relations.py` (shared relation/direction vocab + `query_intent`); `graph_store.py` edges typed from the OKF anchor + a maintained reverse `_in` index + intent-driven traversal; the eval's discrimination slices flip to **victory** (wrong-direction/relation distractor excluded, leak_w 1→0). **Back-compat:** untyped link = `relates_to` generic (traversed by any query both ways) → D008 cascade / profile-matrix / `test_graph_store` / routing all green. No `[CONTRACT]` change; surface/`as_of`/scoring unchanged.
