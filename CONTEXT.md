...
## Current state: core build SHIPPED; routing+embedder slice + write-path arc COMPLETE & MERGED; routed writes are now LIVE on the product path — the plugin consumes a RouterStore via `contract.build_store` (#76, ADR-harness-011, verified in code); remaining gate = a captained benchmark metric run (see Active work)
...
## Active work: routing+embedder slice + write-path arc COMPLETE & MERGED; write-routing now LIVE on the product path — plugin consumes RouterStore via `contract.build_store` (#76/ADR-harness-011); the graph durability→delete→e2e arc + the delete `[CONTRACT]` are MERGED (#89/#92/#93/#95/#99/#101); next solo = Neo4j; remaining gates = captained benchmark run + Keith's plugin `build_store` graph-path line (updated 2026-06-23)
...
- **WHERE STORAGE ACTUALLY LIVES (UPDATED 2026-06-22, #76/ADR-harness-011 — verified in code):** the plugin is now a
  ...
  **markdown → `$MEMORY_STORE/markdown/`** (OKF docs), **graph → in-memory (NO path, NOT persisted — the durability gap the
  next arc closes)**, events → `$MEMORY_STORE/<events>.jsonl`. The engine **auto-selects the profile** with zero plugin
...
- **Resume:** shared `main` @ current (teammate PRs merged since: #60 dataset schema, **#61 PRD-compliance
  ...
  **RouterStore #66 (D025, MERGED)** + the plugin now **consumes it on the live path (#76/ADR-harness-011, verified):** `_Engine.remember` → `store.write` (routed + deduped) via `contract.build_store` → `RouterStore`; dreaming too (#79). So routed writes are **LIVE on the product path** (not just injectable); the harness `MemoryFramework` stubs are bypassed by `build_store` (decide wire-or-retire).
  **Next-session priorities: (1) ⭐ the captained large-benchmark metric run** (real embedder; auto-accuracy-profile when `VOYAGE_API_KEY` is set) — the headline-metrics gate now that the plugin routes through RouterStore; **(2) the current SOLO graph arc** — Step 0 (#75/D029) + Step 1 typed/directional (#81/D030) + Step 1b `okf.py` anchor-capture (#84/D031) + multi_hop `max_depth` (#85/D032) + accuracy-profile depth wiring (#86/D033) + semantic_seed `embed=` hybrid seam (#89/D034) + **graph durability (#92/D035) + delete (#93/D036) + e2e CRUD (#95/D037) + delete `[CONTRACT]` on the protocol (#99+#101/D038) — ALL MERGED**; **Neo4j Phase-A parity floor — ✅ SHIPPED (PR #111, D041; transport = Bolt per D039; Codex R1 FAIL→fold→R2 PASS): the last solo graph thread is DONE.** `Neo4jGraphStore` behind `uri=` reproduces the in-memory store id-set+order EXACTLY (parity by construction), nodes + `okf_links` SSOT only; native typed graph deferred to **Phase B** (scoped in `GRAPH_STORE_SCOPE.md`). Still owed: the **captained live `NEO4J_TEST_URI` run** (Cypher validity vs real Neo4j — flips the live test's captained-pending flag). **(3) THEN — Backend Durability Hardening Arc (D040, NOW NEXT — the graph thread is done):** both durable backends are `needs-hardening`, markdown/OKF is effectively POC persistence (non-atomic writes + no cross-process lock + frozen RAM mirror → silent loss/stale recall on the LIVE path); see `BACKEND_DURABILITY_AUDIT.md` + the queued arc atop `REMEDIATION_PLAN.md`. **(4) Neo4j Phase B** (native typed-graph + GDS accuracy, captained). Cross-team/pending: Keith's plugin `build_store` graph-path line (makes the LIVE plugin graph durable). (`GRAPH_STORE_SCOPE.md`).**
